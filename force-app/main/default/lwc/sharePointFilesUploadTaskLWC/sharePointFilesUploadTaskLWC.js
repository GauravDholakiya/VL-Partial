...
export default class sharepointFileUploadController extends LightningElement {
    ...
    @wire(isTaskWidgetAccessible, { taskId: '$recordId', requestedForPayroll: false })
    ...
    wiredgetTasksWithCustomFields({ error, data }) {
        if (data) {
            ...
            if (data.length > 0) {
               ...
               // console.log('Data--> '+this.data);
                ...
                this.TaskAccountingURL = data[0].Account__r.Accounting_Channel_URL__c;
                this.TaskAccountId = data[0].Account__r.Id;
               console.log('Task AccountID-> ',this.TaskAccountId);
               
               if(typeof this.TaskAccountingURL == "undefined"||this.TaskAccountingURL == null||this.TaskAccountingURL == '')
                 {
                this.TaskAccountingURL = '-AccountingSharedChannel';
                console.log('MAIN CHECK 1',this.TaskAccountingURL);
                 }
                if(data[0].hasOwnProperty("AP_Task__r")){
                this.actionplantask = data[0].AP_Task__r.Subject__c;
                console.log('actionPlanTask', this.actionplantask);
                }
     ...
     @wire(getAllFileDocuments, { recordId: '$TaskAccountId', channelname: '$TaskAccountingURL',TaskNumber:'$taskNumberString', QueryText: '$searchTerm', SortingField: '$sortColumn', SortingDirection: '$sortDirection' })
    ...
    async CheckifFileExistsinSPO() {
        try {
            ...
            const result = await checkforExistingMultipleFilesTask({ recordId: this.recordId, fileNames: JSON.stringify(tempFileNames), channelname:this.TaskAccountingURL });
    ...
    async handleSharePointTokenReceived(message) {
        if (message.token && message.token != "Error") {
            ...
            this.selectedValue = '';
            await uploadFile({ recordId: this.recordId, widgetName: 'Accounting' })
                .then((r) => {
    ...
    async handleUploadWithRESTAPI(token, file, fileContents) {
        ...
        this.url1 = this.createdsiteurl + this.TaskAccountingURL+"/_api/web/GetFolderByServerRelativePath(decodedurl='/sites/" + this.nickname + this.TaskAccountingURL+"/Shared Documents')/Files/AddUsingPath(overwrite=true,decodedurl='" + updatedFileName + "')";
        // Obtain a context info request to get the digest value
        console.log(' File Upload url', this.url1);
        const siteUrlnew = this.createdsiteurl + '/_api/contextinfo';
        console.log('Siteurl', siteUrlnew);
        let _this = this;
        try {
            await fetch(siteUrlnew, {
                ...
                .then((digestValue) => {
                    ...
                    return fetch(this.url1, {
                        ...
                        .then(async (attchResult) => {
                            ...
                            if (_this.files.length == 1) {
                                _this.newUploadedFileUrl = 'https://' + new URL(this.createdsiteurl).hostname + attchResultJson.d.ServerRelativeUrl;
                                console.log('Open url', _this.newUploadedFileUrl);
                            }

                            else if (_this.files.length > 1) {
                            ...
                            return this.GetDocumentIDByServerUrl(token, file)
                                .then(itm => {
                                    ...
                                    console.log('uploadfile obj: ', JSON.stringify(itm));
                                    ...
                                    return this.updateFileMetaData(itm.Id,
                                        metadataBody
                    ...
                    _this.errorMessage = "Error while getting digest value.";
                    _this.showErrorMessage = true;
                    _this.showProgressBarModal = false;
                });
        } catch (error) {
    ...
    GetDocumentIDByServerUrl(token, file) {
        ...
        var reqUrl = this.createdsiteurl + this.TaskAccountingURL+'/_api/web/GetFileByServerRelativePath(decodedurl=\'/sites/' + this.nickname + this.TaskAccountingURL+'/Shared Documents/' + updatedFileName + '\')/ListItemAllFields';
    ...
    updateFileMetaData(ItemId, metaData, digest, token) {
        ...
        return fetch(this.createdsiteurl + this.TaskAccountingURL+'/_api/web/Lists/GetByTitle(\'Documents\')/items(' + ItemId + ')',
            {
                method: "POST", headers:
                {
    ...
    updateProgressBar() {
     ...
     handleViewAll() {
        window.open(this.createdsiteurl + this.TaskAccountingURL+'/Shared Documents', '_blank');
    }
    ...
    {
       console.log('Task Acc Id insert-> ',this.TaskAccountId);
       console.log('Current user id insert->',this.userId);
        CreateLWCRecord({LWCName:'Accounting Upload From Task' ,recordId: this.TaskAccountId,UserId:this.userId })
        .then(result => {
