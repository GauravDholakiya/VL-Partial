...
import getFilteredWebinars from '@salesforce/apex/WebinarController.getFilteredWebinars';
import getUserPermissions from '@salesforce/apex/WebinarController.getUserPermissions';
import getCountryPicklistValues from '@salesforce/apex/WebinarController.getCountryPicklistValues';
import getTypePicklistValues from '@salesforce/apex/WebinarController.getTypePicklistValues';
import Id from '@salesforce/user/Id'

export default class WebinarBox extends LightningElement {
    @track data = true;
    @track webinars;
    @track error;
    ...
    //for sorting the Type and Country column
    @track error;
    sortedDirection = "asc";
    sortedColumn;
    @track arrow;
    ...
    @wire(getFilteredWebinars,{ types: '$selectedTypes', countries: '$selectedCountries', searchTerm: '$searchTerm'})
    wiredWerbinars(result) {
        this.loading = true;
        this.wiredWebinarsResult = result;
        if(result.data) {
            this.webinars = result.data;
    ...
    handleDeleteWebinar(event){
        ...
        .then(() => {
            ...
            refreshApex(this.wiredWebinarsResult)
            .then(() => {
                this.disableOptions();
            });
        })
    ...
    handleSuccess() { //called after edit and add function
        const evt = new ShowToastEvent({
            title: "Success!",
            message: "The webinar has been successfully saved.",
            variant: "success",
        });
    ...
    sort(event){ //sorting function for all columns
        ...
        if(this.sortedColumn === "Video_url__c"){
            this.iconLink = true;
            this.iconDescription = false;
            this.iconDate = false;
            this.iconType = false;
    ...
    disableOptionsCountry(){
        if(this.firstWireCountry == false || this.webinars == undefined){
            return;
        }
        console.log('in disabled optionsCountry');
        const countrySet = [];
        for(var x = 0; x < this.webinars.length; x++){
            var country = JSON.stringify(this.webinars[x].Country__c);
    ...
    //disables options for filter if not in database
    disableOptionsType(){
        if(this.firstWireType == false || this.webinars == undefined){
            return;
        }
        console.log('in disabled optionsType');
        const typeSet = [];
        for(var x = 0; x < this.webinars.length; x++){
            var type = JSON.stringify(this.webinars[x].Type__c);
            typeSet.push(type);
        }
